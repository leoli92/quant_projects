...
def fetch_prices(
    ...
    start / end   : date strings 'YYYY-MM-DD' (defaults to config.DATA_START / OOS_END,
                    i.e. the full warmup + in-sample + out-of-sample range)
    ...
    if not force_download and os.path.exists(cache_file):
        ...
        if prices.empty:
            print("[fetch_data] Cache is empty; re-downloading.")
        else:
    ...
    # Keep only requested tickers that actually downloaded usable (non-empty) data
    ...
    if dropped:
        print(f"[fetch_data] {len(dropped)} tickers failed to download / all-NaN: {dropped}")
